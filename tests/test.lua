...
assert(countKeys(trie:match("sensor/data/humidity")) == 1, "Single level wildcard match failed")

-- Test wildcard ("#") at the end of a key
trie:insert("sensor/data/#", "value3")
assert(countKeys(trie:match("sensor/data/pressure")) == 1, "Multi-level wildcard match failed at the end of a key")
...
assert(countKeys(trie:match("sensor/camera/brightness/level")) == 1, "Single wildcard in the middle of a key match failed")
assert(trie:retrieve("sensor/+/brightness/level") == "value4", "Single wildcard retrieval failed")
trie:delete("sensor/+/brightness/level")
assert(countKeys(trie:match("sensor/camera/brightness/level")) == 0, "Single wildcard deletion failed")

-- Test wildcards ("+" and "#") in the same key
trie:insert("sensor/+/motion/#", "value5")
assert(countKeys(trie:match("sensor/bedroom/motion/detected")) == 1, "Wildcards '+' and '#' in the same key match failed")

-- Test multiple matches
assert(countKeys(trie:match("sensor/data/temperature")) == 2, "Multiple match failed")

-- Test deletion
trie:delete("sensor/data/temperature")
assert(trie:retrieve("sensor/data/temperature") == nil, "Retrieve failed after deletion")
assert(countKeys(trie:match("sensor/data/temperature")) == 1, "Match should still succeed after deletion due to multi-level wildcard")
trie:delete("sensor/data/#")
assert(countKeys(trie:match("sensor/data/temperature")) == 0, "Match should no longer succeed")
